...
├── practice3/async_collector.py  실습3. 모의·실제 HTTP 비동기 수집기
├── Total1/                       종합실습1. 비동기 ETL 파이프라인 + pytest 6개
│   ├── models.py  pipeline.py  test_pipeline.py
├── Advanced/                      창의 과제
│   ├── log_watchdog.py           슬라이딩 윈도우 5xx 이상 탐지
│   └── skala_store_manager.py    SKALA 매점 상품관리 대시보드
├── data/                         실습용 데이터
└── screenshots/                  실행 결과 및 대시보드 캡처
...
python practice3/async_collector.py --real --total 10
...
- 실습3 실제 HTTP: JSONPlaceholder 공개 API 호출 경로 확인
- 종합1: pytest 6개 PASSED, output/에 CSV·Parquet 생성
- Advanced 매점관리: 정상 23 / 오류 3 / 관리자 경고 14건, 테스트 4개 PASSED
